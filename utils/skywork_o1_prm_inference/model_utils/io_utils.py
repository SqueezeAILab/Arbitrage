...
# this is for the router model
def derive_step_rewards_vllm_router(raw_rewards):
    batch_step_rewards = []
    for idx, data in enumerate(raw_rewards.data):
        rewards = data.embedding
        # get the last two values of the rewards
        step_rewards = torch.tensor(rewards[-2:])
        # apply softmax to the last two values
        step_rewards_post_softmax = torch.softmax(step_rewards, dim=-1)[-1].item()
        batch_step_rewards.append(step_rewards_post_softmax)
    return batch_step_rewards
